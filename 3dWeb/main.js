...
function checkCollision() {
  if (playerBB.intersectsBox(testBB) && hasIntersectedBefore){
    ...
    window.location.replace('/Donate/Donate.html')
  }
  else if (playerBB.intersectsBox(outreachBB) && hasIntersectedBefore) {
    window.location.replace("/Outreach/Outreach.html")
  }
  else {
...
const outreachTexture = new THREE.TextureLoader().load('/outreach.png')

const outreachText = new THREE.Mesh(
  new THREE.PlaneGeometry(30, 15),
  new THREE.MeshStandardMaterial({map: outreachTexture, side: THREE.DoubleSide, transparent: true})
)

scene.add(test, sand, ocebots, instructions, aboutUsText, aboutUs, cad, cadText, code, codeText, donate, donateText, outreach, outreachText)
...
outreachText.position.set(-38.5, 5, -90)
outreachText.rotation.y = degToRad(50)
